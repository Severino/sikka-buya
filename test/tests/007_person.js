...
describe(`Person Queries`, function () {
    it(`List`, async function () {
        let result = await graphql(`{person{id,name,shortName,role {
            id
            name
          },dynasty {
            id,
...
`)
    ...
    it("Unauthorized Update Rejected", async function () {
        let promise = graphql(`mutation{updatePerson(data:{id:5, name: "changed"})}`)
        await expect(promise).to.be.rejectedWith(["401"])
    })

    it("Update", async function () {
        let promise = graphql(`mutation{updatePerson(data:{id:5, name: "changed"})}`, {}, TestUser.users[0].token)
        await expect(promise).to.be.fulfilled
    })
